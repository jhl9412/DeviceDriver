...
#define CALL_DEV_NAME	"ledkey_block"
...
static int ledkeyblock_open(struct inode *inode, struct file *filp)
{
...
static loff_t ledkeyblock_llseek(struct file *filp, loff_t off, int whence)
{
	printk("call llseek -> off : %08X, whence : %08X\n",(unsigned int)off, whence);
	return 0x23;
}
static ssize_t ledkeyblock_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
...
static long ledkeyblock_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err;
...
//		printk("time : %ld.%ld\n",INTVAL/100,INTVAL%100);
	...
	ctrl_info.size=(unsigned long)INTVAL;
	sw_no = 0;
...
static ssize_t ledkeyblock_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	char kbuf;
...
static int ledkeyblock_release(struct inode *inode, struct file *filp)
{
	led_write(0);
	printk("call release \n");
	return 0;
...
struct file_operations ledkeyblock_fops = 
{
	.owner	= THIS_MODULE,
	.llseek	= ledkeyblock_llseek,
	.read	= ledkeyblock_read,
	.write	= ledkeyblock_write,
	.unlocked_ioctl	= ledkeyblock_ioctl,
	.open	= ledkeyblock_open,
	.release	= ledkeyblock_release,
};
static int ledkeyblock_init(void)
{
	int result;
	time_old = get_jiffies_64();
	led_init();
	...
	result = register_chrdev(CALL_DEV_MAJOR,CALL_DEV_NAME, &ledkeyblock_fops);
	if(result < 0) return result;
	return 0;
}
static void ledkeyblock_exit(void)
{
...
module_init(ledkeyblock_init);
module_exit(ledkeyblock_exit);
